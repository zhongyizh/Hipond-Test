...
}, n = {}, r = wx.getSystemInfoSync(), h = r.windowWidth, o = r.system, l = t(" ,\r,\n,\t,\f"), c = 0;
...
s.prototype.parse = function(t) {
    ...
    var i = t.attrs, s = (this.tagStyle[t.name] || "").split(";").concat((i.style || "").split(";")), e = {}, a = "";
    ...
    for (var n = 0, r = s.length; n < r; n++) {
        var o = s[n].split(":");
        if (!(o.length < 2)) {
            var c = o.shift().trim().toLowerCase(), d = o.join(":").trim();
    ...
    var s = i.attrs, e = this.stack[this.stack.length - 1], r = e ? e.children : this.nodes, o = this.xml ? t : a.voidTags[i.name];
        ...
        if ("img" === i.name) {
            ...
            !s.ignore || i.webp || s.src.includes("cloud://"))) {
                for (var p = this.stack.length; p--; ) {
                    ...
                    var g = u.attrs.style || "";
                    if (!g.includes("flex:") || g.includes("flex:0") || g.includes("flex: 0") || d.width && d.width.includes("%")) if (g.includes("flex") && "100%" === d.width) for (var f = p + 1; f < this.stack.length; f++) {
                    ...
                    d.width = "") : u.attrs.style += ";max-width:100%"); else {
    ...
    } else ("pre" === i.name || (s.style || "").includes("white-space") && s.style.includes("pre")) && 2 !== this.pre && (this.pre = i.pre = 1), 
    ...
    var t = this.stack.pop(), i = t.attrs, s = t.children, e = this.stack[this.stack.length - 1], n = e ? e.children : this.nodes;
    ...
    a.blockTags[t.name]) t.name = "div"; else if (a.trustTags[t.name] || this.xml) if ("a" === t.name || "ad" === t.name) this.expose(); else if ("video" === t.name || "audio" === t.name) t.children = void 0; else if ("ul" !== t.name && "ol" !== t.name || !t.c) {
        if ("table" === t.name) {
            var p = parseFloat(i.cellpadding), u = parseFloat(i.cellspacing), g = parseFloat(i.border);
                ...
                for (var x = 1; x <= m.length; x++) {
                    for (var b = 1, w = 0; w < m[x - 1].children.length; w++, b++) {
                        ...
                        if ("td" === k.name || "th" === k.name) {
                            for (;y[x + "." + b]; ) b++;
                            k.c = 1;
                            var N = k.attrs.style || "", T = N.indexOf("width") ? N.indexOf(";width") : 0;
                            if (-1 !== T) {
                                var O = N.indexOf(";", T + 6);
    ...
    for (var B in V && (t.f = ";max-width:100%"), c) if (c[B]) {
    ...
    if (this.i = this.content.indexOf("<", this.i), -1 !== this.i) {
        ...
        this.start = ++this.i, this.state = this.tagName; else if ("/" === t || "!" === t || "?" === t) {
            ...
            this.i = this.content.indexOf(s, this.i), -1 !== this.i && (this.i += s.length, 
            this.start = this.i);
        } else this.i++;
    ...
    if (l[t] || "=" === t) {
        ...
        for (var i = "=" === t, s = this.content.length; ++this.i < s; ) if (t = this.content[this.i], 
        !l[t]) {
    ...
    var t = this.content[this.i], i = this.content.length;
    if ('"' === t || "'" === t) {
